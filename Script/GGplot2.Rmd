...
```{r data, exercise = T, exercise.eval = T, exercise.lines = 10}
...
```{r description, exercise = T, exercise.eval=FALSE, exercise.lines = 6}
#Identifiants ?
...
```{r hist1, echo=TRUE, message=FALSE, warning=FALSE}
zoo %>% ggplot(aes(x = Number.ind)) +
  geom_histogram()
```

```{r hist2, exercise = T, exercise.eval = T}
zoo %>% ggplot(aes(x = Number.ind)) +
...
```{r box2, exercise = T, exercise.eval = T}
...
```{r vio1, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r vio2, message=FALSE, warning=FALSE, exercise=T, exercise.eval=T}
...
```{r bar2, exercise = T, exercise.eval = T}
...
```{r xyplot1, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r xyplot2, message=FALSE, warning=FALSE, exercise=T, exercise.eval=T}
...
```{r lmplot1, message=FALSE, warning=FALSE, exercise=T, exercise.eval=T}
...
```{r lmplot2, message=TRUE, warning=FALSE, exercise=T, exercise.eval=T}
...
```{r lmplot3, exercise = T, exercise.eval = T}
zoo %>% ggplot(aes(x = Number.ind, y = Food.kg, col = Animal)) +
  geom_point() +
  xlab("Nombre d'individus") +
  ylab("Nourriture consommée (kg) (log10 scale)") +
...
```{r lmplot4, exercise = T, exercise.eval = T}
zoo %>% ggplot(aes(x = Number.ind, y = Food.kg, col = Animal)) +
  geom_point() +
  xlab("Nombre d'individus") +
  ylab("Nourriture consommée (kg) (log10 scale)") +
...
```{r theme, exercise = T, exercise.eval = T}
zoo %>% ggplot(aes(x = Animal, y = Number.ind)) +
  geom_boxplot() +
  theme_classic()
```
...
```{r thememodif, exercise = T, exercise.eval = T}
zoo %>% ggplot(aes(x = Animal, y = Number.ind)) +
  geom_boxplot() +
  theme_classic() +
...
```{r interactif1, exercise = T, exercise.eval = T}
ggplot_bar <- zoo %>% 
  ggplot(aes(x = Animal, y = Size.cm)) +
  geom_bar(stat = "summary", fun.y = "mean", aes(fill = Animal, col = Animal), alpha = 0.6) +
  xlab("Animal") +
...
```{r interactif2, exercise = T, exercise.eval = T}
...
```{r map1, exercise = T, exercise.eval = T, exercise.lines = 25}
